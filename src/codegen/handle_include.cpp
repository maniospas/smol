...
bool handle_include(map<string, Types>& files, string file,
    ...
    bool errors = codegen(files, path, builtins, selected_task, task_report);
    ...
    if (files[path].all_errors.size() && Def::markdown_errors)
