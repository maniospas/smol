...
string Def::recommend_runtype(const Types& types, const Variable& candidate) {
    ...
    for(const auto& it : types.vars) {
        int distance = sellersMinimumEditDistance(candidate.to_string(), it.first.to_string())+sellersMinimumEditDistance(it.first.to_string(), candidate.to_string());
...
string Def::recommend_variable(const Types& types, const Variable& candidate) {
    ...
    for(const auto& it : internalTypes.vars) if(!released[it.first]) {
        int distance = sellersMinimumEditDistance(candidate.to_string(), it.first.to_string())+sellersMinimumEditDistance(it.first.to_string(), candidate.to_string());
        if(distance<min_distance) {min_distance = distance;recommendation = it.first.to_string();}
    }
    if(!recommendation.size()) return recommendation;
    if(recommendation.size()>=2 && recommendation[0]=='_' && recommendation[1]=='_') return "";
