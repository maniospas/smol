...
Variable Def::next_var_buffer_ret_at(Variable next, const shared_ptr<Import>& i, size_t& p, const Variable& first_token, Types& types, bool test) {
    if(active_calls.find(next)!=active_calls.end() && active_calls[next].exists()) {
        if(active_calls[active_calls[next]].exists()) {
            ...
            errors += Code(
                fail_var, 
                token_print, 
                contains(next)?vars[next]->name:EMPTY_VAR, 
                call_var, 
                token_failsafe
            );
    ...
    vars[next+vars[next]->packs[1]] = types.vars[BUFFER_VAR];
    buffer_types[next+vars[next]->packs[1]] = vars[next]->buffer_types[vars[next]->packs[1]];
    if(!buffer_types[next+vars[next]->packs[1]])
        imp->error(--p, "Internal error: Failed to transfer a buffer type");
    implementation += Code(
        next+vars[next]->packs[1],
        ASSIGN_VAR,
        MUL_VAR,
